...
class BlockLibrary:
    ...
    def load_library_file(self, library_path: str, use_frequency: True) -> Tuple[List[SMILES], FloatTensor]:
    ...
    @classmethod
    def decompose(cls, mol) -> Optional[List[SMILES]]:
        try:
            result = cls.fragmentation.decompose(mol)
            return result
        except Exception:
            return None
