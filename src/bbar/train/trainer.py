...
class Trainer:
    ...
    def setup_data(self, args):
        ...
        self.train_dataloader = PyGDataLoader(
            self.train_dataset, args.train_batch_size, num_workers=args.num_workers, shuffle=True, drop_last=True
        )

        self.val_dataloader = PyGDataLoader(
            self.val_dataset, args.val_batch_size, num_workers=args.num_workers, shuffle=True, drop_last=True
        )
    ...
    def property_prediction(self, Z_core: GraphVector, cond: PropertyVector, y_term: BoolTensor) -> LossScalar:
    ...
    def condition_embedding(
        self, x_upd_core: NodeVector, Z_core: GraphVector, cond: PropertyVector, node2graph_core: LongTensor
    ...
    def building_block_embedding(self, block: Union[PyGBatch, LongTensor], train: bool) -> GraphVector:
        if train:
    ...
    def building_block_priority(
        self, Z_core: GraphVector, Z_block: GraphVector, y_add: BoolTensor, sample_type: str
    ) -> LossScalar:
